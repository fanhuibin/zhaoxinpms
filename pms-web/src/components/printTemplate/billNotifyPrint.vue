            ...
            <table class="body" cellpadding="0" cellspacing="0" style="width: 99%">
                ...
                <tbody>
                    <tr v-for="(item, i) in data.bills" :key="i">
                        ...
                        <td align="center">{{ parseTime(item.deadline, '{y}-{m}-{d}') }}</td>
                        <td align="right">{{ item.total }}</td>
                        <td align="right">{{ item.lateFee }}</td>
                    </tr>
                </tbody>
...
export default {
    ...
    methods: {
        ...
        print(resourceName, feeItemId) {
            this.getData(resourceName, feeItemId).then(res => {
                this.loaded = true;
                this.data = res.data;
                this.$nextTick(() => {
                    var strBodyStyle =
