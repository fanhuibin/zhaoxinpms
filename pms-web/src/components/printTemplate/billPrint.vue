        ...
        <div id="print-body" ref="print-body">
            <table class="body" cellpadding="0" cellspacing="0" style="width: 99%">
                <thead>
                    <tr>
                        <th width="30%">收费项名</th>
                        <th width="10%">费用开始时间</th>
                        <th width="10%">费用结束时间</th>
                        <th width="10%">单价</th>
                        <th width="10%">数量</th>
                        <th width="15%">总价</th>
                        <th width="15%">实收</th>
                ...
                <tbody>
                    <tr v-for="(item, i) in data.bills" :key="i">
                        <td>{{ item.feeItemName }}</td>
                        <td align="center">{{ parseTime(item.beginDate, '{y}-{m}-{d}') }}</td>
                        <td align="center">{{ parseTime(item.endDate, '{y}-{m}-{d}') }}</td>
                        <td align="right">{{ item.price }}</td>
                        <td align="center">{{ item.num }}</td>
                        <td align="right">{{ item.total }}</td>
                        <td align="right">{{ item.receivable }}</td>
                    </tr>
                </tbody>
                ...
                <tfoot>
                    ...
                    <tr>
                        ...
                        <td></td>
                        <td></td>
                        <td align="right" format="">{{ data.payLog.itemTotalMoney }}</td>
                        <td align="right">{{ data.payLog.receivableMoney }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
        <div id="print-footer" ref="print-footer">
            <table class="footer" cellpadding="0" cellspacing="0" style="width: 99%">
                <tr>
                    <td width="40%">
                        <b>滞纳金：</b>
                        {{ data.payLog.lateFeeMoney }}
                    </td>
                    <td>
                        <b>优惠：</b>
                        {{ data.payLog.discountMoney }}
                    </td>
                    <td>
                        <b>应收：</b>
                        {{ data.payLog.receivableMoney }}
                    </td>
                </tr>
                <tr>
                    <td>
                        <b>实付金额：</b>
                        {{ data.payLog.payMoney }}
                    </td>
                    <td>
                        <b>预存款支付：</b>
                        {{ data.payLog.prePayMoney }}
                    </td>
                    <td>
                        <b>支付找零：</b>
                        {{ data.payLog.changeMoney }}
                    </td>
                </tr>
                <tr>
                    <td>
                        <b>找零结转：</b>
                        {{ data.payLog.preSaveMoney }}
                    </td>
                </tr>
            </table>
        </div>
...
export default {
    ...
    methods: {
        getData(payLogId) {
            return request({
                url: '/print/printData/billPrint?payLogId=' + payLogId,
                method: 'get',
            });
        },
        print(payLogId) {
            this.getData(payLogId).then(res => {
                this.loaded = true;
                this.data = res.data;
                this.$nextTick(() => {
                    var strBodyStyle =
